...
--获取所有性别分类商品数据
select p.product_id,p.image,p.price,p.is_special_offer,p.special_price,p.gender,p.style,p.ordered_num,s.series_name,b.chinese_name,b.grade from products p left join series s on p.series_id = s.series_id left join brands b on s.brand_id = b.brand_id where gender = "男";

--获取限定商品数据
select p.product_id,p.image,p.price,p.is_special_offer,p.special_price,p.gender,p.style,p.ordered_num,s.series_name,b.chinese_name,b.grade from products p left join series s on p.series_id = s.series_id left join brands b on s.brand_id = b.brand_id where p.is_new = 1 limit 4;

--获取商品详细信息
select p.*,s.series_name,b.chinese_name,b.english_name,,b.grade,b.brand_id from products p left join series s on p.series_id = s.series_id left join brands b on s.brand_id = b.brand_id where p.product_id = ;

--获取品牌热销商品数据
select p.product_id,p.style,p.price,p.is_special_offer,p.special_price,p.image,p.ordered_num,s.series_name,b.chinese_name,b.english_name from products p left join series s on p.series_id = s.series_id left join brands b on s.brand_id = b.brand_id where b.brand_id = ? order by p.ordered_num desc limit 4;